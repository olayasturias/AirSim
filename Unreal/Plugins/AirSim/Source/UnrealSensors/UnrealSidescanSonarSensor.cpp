...
#include "UnrealSidescanSonarSensor.h"
...
// ctor
UnrealSidescanSonarSensor::UnrealSidescanSonarSensor(const AirSimSettings::SidescanSonarSetting& setting,
                                     AActor* actor, const NedTransform* ned_transform)
    : SidescanSonarSimple(setting), actor_(actor), ned_transform_(ned_transform)
{
    createLasers();
}

// initializes information based on sidescansonar configuration
void UnrealSidescanSonarSensor::createLasers()
{
    msr::airlib::SidescanSonarSimpleParams params = getParams();
...
void UnrealSidescanSonarSensor::getPointCloud(const msr::airlib::Pose& sidescansonar_pose, const msr::airlib::Pose& vehicle_pose,
                                      const msr::airlib::TTimeDelta delta_time, msr::airlib::vector<msr::airlib::real_T>& point_cloud, msr::airlib::vector<int>& segmentation_cloud)
{
    ...
    if (total_points_to_scan > MAX_POINTS_IN_SCAN) {
        ...
        UAirBlueprintLib::LogMessageString("SidescanSonar: ", "Capping number of points to scan", LogDebugLevel::Failure);
    }
    ...
    if (points_to_scan_with_one_laser <= 0) {
        //UAirBlueprintLib::LogMessageString("SidescanSonar: ", "No points requested this frame", LogDebugLevel::Failure);
        return;
    }
    ...
    for (auto laser = 0u; laser < number_of_lasers; ++laser) {
        ...
        for (auto i = 0u; i < points_to_scan_with_one_laser; ++i) {
            ...
            if (shootLaser(sidescansonar_pose, vehicle_pose, laser, horizontal_angle, vertical_angle, params, point, segmentationID)) {
...
bool UnrealSidescanSonarSensor::shootLaser(const msr::airlib::Pose& sidescansonar_pose, const msr::airlib::Pose& vehicle_pose,
...
{
    // start position
    Vector3r start = VectorMath::add(sidescansonar_pose, vehicle_pose).position;
    ...
    // get ray quaternion in sidescansonar frame (angles must be in radians)
    ...
    msr::airlib::Quaternionr ray_q_b = VectorMath::coordOrientationAdd(ray_q_l, sidescansonar_pose.orientation);
    ...
    if (is_hit) {
        ...
        if (false && UAirBlueprintLib::IsInGameThread()) {
            ...
            // Mostly shouldn't be needed. Use SimModeBase::drawSidescanSonarDebugPoints()
            DrawDebugPoint(
                actor_->GetWorld(),
        ...
        if (params.data_frame == AirSimSettings::kVehicleInertialFrame) {
            ...
            // tranform to sidescansonar frame
            point = VectorMath::transformToBodyFrame(point_v_i, sidescansonar_pose + vehicle_pose, true);

            // The above should be same as first transforming to vehicle-body frame and then to sidescansonar frame
            //    Vector3r point_v_b = VectorMath::transformToBodyFrame(point_v_i, vehicle_pose, true);
            //    point = VectorMath::transformToBodyFrame(point_v_b, sidescansonar_pose, true);

            // On the client side, if it is needed to transform this data back to the world frame,
            // then do the equivalent of following,
            //     Vector3r point_w = VectorMath::transformToWorldFrame(point, sidescansonar_pose + vehicle_pose, true);
            // See SimModeBase::drawSidescanSonarDebugPoints()
